...
│       │       ├── engine.health_check()  [Function] ----> Check LLM + pgvector status
│       │       ├── [Conditional Branch] -----------------> llm_healthy AND pgvector_healthy?
│       │       │       ├── True  -----------------------> status = "healthy"
...
def progress_update(step: str, substep: str, status: str = "...", details: str = ""):
    ...
    msg = f"[{timestamp}] [{status}] {step} - {substep} {details}"
    try:
        print(msg)
...
def ask(session_id: str, req: AskRequest):
    ...
    try:
        ...
        progress_update("RAG.4", "Vector Retrieval", "⏳", "Searching pgvector for relevant vectors...")
...
def ask_stream(
    ...
    def generate():
        try:
            ...
            progress_update("STREAM.4", "Vector Retrieval", "⏳", "Searching pgvector...")
...
def health():
    ...
    try:
        ...
        progress_update("HEALTH.2", "Component Test", "⏳", "Testing LLM and pgvector...")
        details = engine.health_check()
        progress_update("HEALTH.2", "Component Test", "✅", "Components tested")
        status = "healthy" if details.get("llm_healthy") and details.get("pgvector_healthy") \
            else "degraded"
