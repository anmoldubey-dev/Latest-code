...
# ═══════════════════════════════════════════════════════════════════
# DEFAULT CONFIGURATION - Automatically uses vector_store from main.py
# ═══════════════════════════════════════════════════════════════════

COLLECTION_NAME = "vector_store"  # pgvector table created by main.py
PGVECTOR_TABLE  = "vector_store"  # pgvector table name
...
DEFAULT_SOURCE_TABLE      = os.getenv("PG_TABLE", "users")  # Source table from .env
DEFAULT_TARGET_TABLE      = f"{os.getenv('PG_TABLE', 'users')}_extracted"  # Auto-generate target name
...
for _n in [
    "httpx", "httpcore", "huggingface_hub", "pgvector",
    "psycopg2",
...
def progress_update(step: str, substep: str, status: str = "⏳", details: str = "") -> None:
    ...
    if duration_info:
        console.print(
            ...
            f"[bold]{step}[/] → [cyan]{substep}[/] {duration_info} {escape(details)}"
        )
    else:
        console.print(
            f"[dim]{timestamp}[/] [{status_color}]{status}[/] "
...
def run(args: argparse.Namespace) -> int:
    progress_update("STARTUP", "run()", "⏳", "Starting HAUP v3.0 Reverse Pipeline...")

    # Determine source and target
    source_table = args.source_table or DEFAULT_SOURCE_TABLE
    target_table = args.output_table or f"{source_table}_extracted"
    
    console.print(Panel(
        "[bold white]HAUP v3.0[/]  ─  [cyan]Vector DB → PostgreSQL Reverse Extractor[/]\n"
        "[dim]Automatically extracts data from vector_store to PostgreSQL[/]\n\n"
        f"[bold]Source:[/] [cyan]vector_store[/] (pgvector table from main.py)\n"
        f"[bold]Target:[/] [cyan]{target_table}[/] (reconstructed PostgreSQL table)\n"
        f"[bold]Schema:[/] [cyan]{source_table}[/] (original table structure)\n\n"
        "[dim]Resumable exports with checkpoint tracking  │  Cost: $0.00[/]",
        border_style = "bright_blue",
        padding      = (0, 1),
    ))

    # ── Step 1: pgvector ─────────────────────────────────────────
    progress_update("STEP 1", "from pgvector_client import PgvectorClient", "⏳", "Loading pgvector...")
    try:
        from pgvector_client import PgvectorClient
        progress_update("STEP 1", "from pgvector_client import PgvectorClient", "✅", "pgvector loaded")
    except ImportError:
        progress_update("STEP 1", "from pgvector_client import PgvectorClient", "❌", "pgvector not installed")
        console.print("[red]ERROR:[/] pip install pgvector")
        return 1

    # ── Step 2: Core modules ─────────────────────────────────────
    progress_update("STEP 2", "from reverse_core import *", "⏳", "Importing reverse pipeline modules...")
    ...
    progress_update("STEP 2", "from reverse_core import *", "✅", "All modules imported")

    # ── Step 3: Hardware detection ───────────────────────────────
    progress_update("STEP 3", "hardware_detector.detect()", "⏳", "Detecting system capabilities...")
    try:
    ...
    progress_update("STEP 3", "hardware_detector.detect()", "✅",
                    f"{config.num_workers} workers, chunk_size={config.chunk_size}")

    # ── Step 4: pgvector connection ──────────────────────────────
    progress_update("STEP 4", "PgvectorClient()", "⏳", "Connecting to vector database...")
    console.print("\n[bold bright_blue]Data Source[/]  " + "─" * 45)
    console.print(f"  [bold]Table[/]                  [cyan]{args.collection}[/]")
    console.print(f"  [bold]pgvector Database[/]      [cyan]{os.getenv('PGVECTOR_DATABASE', 'vector_db')}[/]")
    ...
    progress_update("STEP 4", "PgvectorClient()", "✅", "Connected")

    # ── Step 5: Collection stats ─────────────────────────────────
    progress_update("STEP 5", "vect_batch_reader.get_collection_stats()", "⏳", "Analysing collection...")
    stats = vect_batch_reader.get_collection_stats(client, args.collection)
    if stats.total_entries == 0:
        progress_update("STEP 5", "vect_batch_reader.get_collection_stats()", "❌", "Collection is empty or missing")
        console.print(f"[red]ERROR:[/] Collection '{args.collection}' is empty or does not exist.")
        return 1
    progress_update("STEP 5", "vect_batch_reader.get_collection_stats()", "✅", f"{stats.total_entries:,} entries found")

    if not stats.has_documents:
        progress_update("STEP 5", "stats.has_documents", "❌", "No documents stored in collection")
        console.print(
    ...
    progress_update("STEP 6", "schema_loader.load()", "⏳", "Loading saved schema strategy...")
    ...
    progress_update("STEP 6", "schema_loader.load()", "✅", "Schema strategy loaded")

    # ── Step 7: Type inference ───────────────────────────────────
    progress_update("STEP 7", "schema_loader.infer_sql_types()", "⏳", "Analysing sample data for SQL types...")
    source_conn = None
    if args.pg_connection_string and args.source_table:
        try:
            import psycopg2
            source_conn = psycopg2.connect(args.pg_connection_string)
            progress_update("STEP 7", "psycopg2.connect()", "⏳",
                            f"Using source table '{args.source_table}' on Neon")
        except Exception as exc:
            progress_update("STEP 7", "psycopg2.connect()", "⚠️",
    ...
    progress_update("STEP 7", "schema_loader.infer_sql_types()", "✅", "Column types inferred")

    # ── Step 8: Constraints ──────────────────────────────────────
    constraints = None
    if args.preserve_constraints and source_conn and args.source_table:
        progress_update("STEP 8", "read_postgresql_constraints()", "⏳", "Loading original table constraints...")
        ...
        try:
            ...
            progress_update("STEP 8", "read_postgresql_constraints()", "✅",
            ...
            progress_update("STEP 8", "read_postgresql_constraints()", "⚠️", f"Failed: {str(exc)[:80]}")
            console.print(f"  [yellow]WARNING:[/] Could not read constraints: {escape(str(exc))}")
    else:
        progress_update("STEP 8", "read_postgresql_constraints()", "⏭️", "Skipped (--preserve-constraints not set or no source)")
    ...
    progress_update("STEP 9", "schema_reconciler.reconcile()", "⏳", "Merging strategy, types, and constraints...")
    schema             = schema_reconciler.reconcile(strategy, type_map)
    schema.constraints = constraints
    progress_update("STEP 9", "schema_reconciler.reconcile()", "✅", "Schema reconciled")
    ...
    if summary.failed > 0:
        progress_update("STEP 10", "checkpoint.retry_failed_chunks()", "⏳",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description     = "HAUP v3.0 — pgvector to PostgreSQL/Excel reverse extraction\n"
                          "Run without arguments to use defaults from .env",
        formatter_class = argparse.RawDescriptionHelpFormatter,
    )

    # pgvector
    p.add_argument("--collection",  default=COLLECTION_NAME,
                   help=f"pgvector table name (default: {COLLECTION_NAME})")
...
if __name__ == "__main__":
    ...
    # Auto-use environment variables if not provided
    if not args.pg_connection_string:
        args.pg_connection_string = os.getenv("NEON_CONNECTION_STRING", "")
