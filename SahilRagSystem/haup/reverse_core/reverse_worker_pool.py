        ...
        while retry_count < max_retries and not success:
            ...
            try:
                for i in range(len(chunk.ids)):
                    ...
                    # Extract version and timestamp data
                    version = chunk.versions[i] if chunk.versions and i < len(chunk.versions) else 1
                    created_at = chunk.created_ats[i] if chunk.created_ats and i < len(chunk.created_ats) else None
                    updated_at = chunk.updated_ats[i] if chunk.updated_ats and i < len(chunk.updated_ats) else None
                    ...
                    if row is not None:
                        row["__rowid__"] = rowid
                        row["__embedding_version"] = version
                        row["__embedding_created_at"] = created_at
                        row["__embedding_updated_at"] = updated_at
                        rows_out.append(row)
                    else:
                        parse_fails += 1
