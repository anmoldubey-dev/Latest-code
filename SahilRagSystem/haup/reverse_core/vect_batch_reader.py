"""
File Summary:
Streams all entries from a pgvector table in fixed-size pages.
...
│       │
│       ├── SELECT COUNT(*) ------------------------------> Total entry count
│       │
│       ├── [Early Exit Branch] total == 0 ---------------> Return empty CollectionStats
│       │
│       ├── SELECT * LIMIT 1 -----------------------------> Sample first entry for field detection
│       │
│       └── Returns CollectionStats  [Class → Object] ----> total_entries, has_documents, sample_metadata
│
├── stream_chunks()  [Function] --------------------------> Generator: paginate pgvector via LIMIT/OFFSET
│       │
│       ├── SELECT * LIMIT/OFFSET ------------------------> Fetch one page of entries
│       │
...
def get_collection_stats(client, collection_name: str) -> CollectionStats:
    """
    Opens the pgvector table and returns stats without reading all data.
...
def stream_chunks(
    ...
    Uses pgvector LIMIT/OFFSET pagination.
    Never loads the full collection into RAM.
    Includes version and timestamp tracking.
    """
    include_fields = ["documents", "metadatas", "versions", "timestamps"]
    ...
    while True:
        ...
        versions = result.get("versions") or [1] * len(ids)
        created_ats = result.get("created_ats") or [None] * len(ids)
        updated_ats = result.get("updated_ats") or [None] * len(ids)
