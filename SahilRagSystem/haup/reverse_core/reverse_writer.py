    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set() or not self.result_q.empty():
            ...
            for row in packet.rows:
                ...
                # Keep version and timestamp fields as they are already in the row
                buffer.append(row)
    ...
    def _init_sql_table(self, target: SQLTarget) -> None:
        ...
        sql = f"""
            CREATE TABLE IF NOT EXISTS "{target.table_name}" (
                ...
                __embedding_version INTEGER,
                __embedding_created_at TIMESTAMP,
                __embedding_updated_at TIMESTAMP,
                __extracted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """
        ...
        cursor.execute(
            f'CREATE INDEX IF NOT EXISTS idx_{target.table_name}_extracted_at '
            f'ON "{target.table_name}" (__extracted_at DESC)'
        )
        cursor.execute(
            f'CREATE INDEX IF NOT EXISTS idx_{target.table_name}_embedding_updated '
            f'ON "{target.table_name}" (__embedding_updated_at DESC)'
        )
        
        target.conn.commit()
        cursor.close()

        logger.info("[ReverseWriter] PostgreSQL table ready with version tracking: %s", target.table_name)
    """================= End method _init_sql_table ================="""

    """================= Startup method _flush_sql ================="""
    def _flush_sql(self, buffer: list[dict], chunk_id: int) -> None:
        target     = self.target
        final_cols = self.schema.final_cols + [
            "__orig_rowid__",
            "__embedding_version",
            "__embedding_created_at",
            "__embedding_updated_at"
        ]
