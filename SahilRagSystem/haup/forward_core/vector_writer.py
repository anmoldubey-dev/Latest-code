"""
File Summary:
Async Vector DB writer for HAUP v3.0. Daemon thread that buffers 2000 vectors
before each bulk upsert to pgvector. Decouples embedding throughput from DB I/O
...
│       ├── vector_db.upsert() ---------------------------> Write batch to pgvector
...
class VectorWriter:
    """
    Daemon thread that buffers vectors and writes to pgvector.
    ...
    def _flush_buffer(self, buffer: list, chunk_ids: set) -> None:
        ...
        """
        try:
            # Deduplicate by ID to avoid pgvector duplicate ID errors
            seen_ids = set()
            deduplicated_buffer = []
            for item in buffer:
                item_id = item['id']
            ...
            logger.debug(f"Successfully upserted {len(deduplicated_buffer)} vectors to pgvector")
