...
_ID_PATTERNS   = re.compile(r'(^|_)(id|pk|rowid|uuid|serial|key|code|number|num|no)($|_)', re.IGNORECASE)
_DATE_PATTERNS = re.compile(r'(date|time|created|modified|timestamp|updated|at)', re.IGNORECASE)
...
class SchemaAnalyzer:
    ...
    def analyze(self,
        ...
        semantic_cols = []
        numeric_cols  = []
        bool_cols     = []  # Add boolean columns tracking
        ...
        for col in columns:
            if _ID_PATTERNS.search(col):
                ...
                if dtype == 'bool':
                    bool_cols.append(col)  # Track boolean columns
        ...
        template = self._build_template(semantic_cols, numeric_cols, id_cols, date_cols, bool_cols)
    ...
    def _build_template(self, semantic_cols, numeric_cols, id_cols, date_cols, bool_cols=None) -> str:
        """
        Build template including ALL columns that should be embedded or stored.
        Now includes id_cols, date_cols, and bool_cols to ensure data completeness for reverse extraction.
        """
        ...
        # Include date columns (created_at, updated_at, etc.)
        embed_cols += date_cols
        
        # Include boolean columns (is_active, is_deleted, etc.)
        embed_cols += bool_cols
