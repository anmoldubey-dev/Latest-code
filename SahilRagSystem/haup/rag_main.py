...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--pgvector-table", default=None)
...
def build_engine(args: argparse.Namespace) -> RAGEngine:
    ...
    if args.pgvector_table:
        cfg.pgvector.table = args.pgvector_table
    if args.collection:
...
def run_interactive(engine: RAGEngine, stream: bool) -> None:
    progress_update("INTERACTIVE", "engine.new_session()", "⏳", "Starting interactive session...")
    progress_update("INTERACTIVE.1", "engine.new_session()", "⏳", "Generating session ID...")
    session_id = engine.new_session()
    progress_update("INTERACTIVE.1", "engine.new_session()", "✅", f"Session ID: {session_id[:8]}...")
    progress_update("INTERACTIVE.2", "console.input()", "✅", "Terminal UI ready")
    progress_update("INTERACTIVE", "engine.new_session()", "✅", f"Interactive session ready")
    ...
    while True:
        ...
        if raw.lower() in ("/quit", "/exit", "/q"):
            ...
            progress_update("INTERACTIVE.HEALTH.1", "Component Check", "⏳", "Testing LLM and pgvector...")
            ...
            progress_update("INTERACTIVE.Q.2", "Vector Retrieval", "⏳", "Searching pgvector...")
