...
# TtsAudioSource — optional livekit-rtc feature
class TtsAudioSource:
    def start(self): pass
    async def push_tts_wav(self, wav_bytes): pass
    source = None

try:
    from routing_ivr.tts_engine import synthesize as _piper_sync_raw
    async def _run_piper(text, voice=""):
        import asyncio
        return await asyncio.get_event_loop().run_in_executor(None, _piper_sync_raw, text)
...
from ai_assist.modes import AIMode
...
class AIJoinManager:
    ...
    async def _ai_assist_task(self, session: AIAssistSession, source: str = "browser"):
        ...
        if rtc is None:
            logger.warning("[AI Assist] livekit-rtc not installed — AI join skipped for room %s", session.room_id)
            async with self._lock:
                self.active_sessions.pop(session.room_id, None)
            return
    ...
    async def _process_utt(self, pcm, session: AIAssistSession):
        logger.debug("Executing AIJoinManager._process_utt")

        if stt_sync is None or not _NP_AVAILABLE:
            return
