...
async def agent_end_call(req: AgentEndCallRequest):
    ...
    # NEW: Actively fail any lingering in_progress outbound call for this agent 
    # to instantly trigger the "We tried to reach you" email.
    pool = await db.get_pool()
    async with pool.acquire() as conn:
        outbound = await conn.fetchrow(
            "SELECT id, user_email, department FROM outbound_queue WHERE assigned_agent=$1 AND status='in_progress' LIMIT 1",
            req.agent_identity
        )
        if outbound:
            ...
            try:
                from .email_service import send_outbound_no_answer_email
                await send_outbound_no_answer_email(outbound["user_email"], outbound["department"])
                
                # Optionally tell frontend that it was cancelled 
                await event_hub.publish({
                    "type":       "outbound_cancelled",
                    "user_email": outbound["user_email"],
                })
            except Exception as e:
                logger.error(f"Failed to trigger no-answer email on agent end-call: {e}")
