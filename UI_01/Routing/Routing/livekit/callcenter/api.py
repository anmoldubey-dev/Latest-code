...
class QueueWaitTimesRequest(BaseModel):
    # keys: "1","2",... and "default" → seconds (int)
    wait_times: dict
...
async def outbound_resume(req: OutboundResumeRequest):
    """Clear ignore_outbounds_until for agent and re-queue recent no_answer items."""
    pool = await db.get_pool()
    async with pool.acquire() as conn:
        agent = await conn.fetchrow(
            "SELECT department FROM agent_states WHERE agent_identity = $1", req.agent_identity
        )
        await conn.execute(
            """
            UPDATE agent_states
            SET status = 'online', ignore_outbounds_until = NULL,
                ignore_reason = '', last_heartbeat = NOW()
...
@cc_router.get("/admin/queue-wait-times")
async def get_queue_wait_times():
    """Return position-based queue wait times set by superuser."""
    import json
    raw = await db.get_config("queue_wait_times")
    if raw:
        try:
            wait_times = json.loads(raw)
        except Exception:
            wait_times = {"default": 300}
    else:
        wait_times = {"default": 300}
    return {"wait_times": wait_times}


@cc_router.post("/admin/queue-wait-times")
async def set_queue_wait_times(req: QueueWaitTimesRequest):
    """Save position-based queue wait times. Superuser only."""
    import json
    # Validate: all values must be positive integers (seconds)
    sanitized = {}
    for k, v in req.wait_times.items():
        try:
            sanitized[str(k)] = max(1, int(v))
        except (ValueError, TypeError):
            pass
    if not sanitized:
        raise HTTPException(status_code=400, detail="wait_times must not be empty")
    await db.set_config("queue_wait_times", json.dumps(sanitized))
    return {"status": "saved", "wait_times": sanitized}
