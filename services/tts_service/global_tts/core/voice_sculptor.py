...
#    |
#    |----> _mix_room_ambience()
#    |        * blend call centre room WAV
#    |
#    |----> _apply_eq()
#    |        * boost 400 Hz warmth, 4 kHz clarity
#    |
#    |----> _loudness_normalize()
#    |        * normalize output to -24 LUFS
#    |
#    v
# +-------------------------------+
# | crossfade()                   |
...
#    |
#    v
# [ RETURN processed float32 audio ]
#
# ================================================================
...
class HumanVoiceSculptor:
    ...
    def _load_room_audio(self):
        ...
        if not room_file.exists():
            logger.warning(
                ...
                "Drop the file into human_tts/assets/ to enable it."
            )
            return
        try:
    ...
    def _mix_room_ambience(self, speech: np.ndarray, sr: int) -> np.ndarray:
        ...
        snr_db = np.random.uniform(20, 25)
        # Use only non-silent samples for RMS so silence gaps don't dilute
        # speech_rms and make room noise relatively too loud mid-speech.
        active = speech[np.abs(speech) > 0.01]
        speech_rms = np.sqrt(np.mean(active ** 2) + 1e-9) if len(active) > 0 else np.sqrt(np.mean(speech ** 2) + 1e-9)
    ...
    # ----------------------------------------------------------
    # Main entry point
    # Pipeline: normalize → room ambience → EQ → loudness normalize
    # ----------------------------------------------------------
    def process(self, audio: np.ndarray, emotion: str = "neutral",  # noqa: ARG002
                sr: int = SAMPLE_RATE) -> np.ndarray:
        audio = audio.astype(np.float32)

        # 1. Peak normalize raw TTS audio
        ...
        # 4. Loudness normalize to -24 LUFS
