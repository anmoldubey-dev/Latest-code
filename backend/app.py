...
from backend.translation_layer import tl_translate_user, tl_translate_agent, select_voice_by_gender
...
from backend.api.crm_routes import crm_router
from backend.api.zoho_routes import zoho_router
...
@app.post("/api/translation/relay")
async def translation_relay(request: Request):
    """Translate text and return TTS audio — used by human LiveKit call translation layer."""
    body = await request.json()
    text       = body.get("text", "")
    from_lang  = body.get("from_lang", "en")
    to_lang    = body.get("to_lang", "en")
    voice_gender = body.get("voice_gender", "female")

    if not text:
        return JSONResponse({"translated_text": "", "audio_b64": ""})

    loop = asyncio.get_running_loop()
    translated = await tl_translate_agent(text, from_lang, to_lang, loop)

    registry = _m.get("voice_registry", {})
    voice    = select_voice_by_gender(registry, to_lang, voice_gender)
    try:
        wav_bytes = await tts(translated, to_lang, voice or "")
        audio_b64 = base64.b64encode(wav_bytes).decode() if wav_bytes else ""
    except Exception as exc:
        logger.warning("[TL relay] TTS failed: %s", exc)
        audio_b64 = ""

    return JSONResponse({"translated_text": translated, "audio_b64": audio_b64})
...
async def ws_call(ws: WebSocket):
    ...
    _tl      = init.get("translation_layer") or {}
    tl_on    = bool(_tl.get("enabled")) and _tl.get("from_lang") != _tl.get("to_lang")
    tl_from  = _tl.get("from_lang", lang)
    tl_to    = _tl.get("to_lang",   lang)
    tl_gen   = _tl.get("voice_gender", "female")
    if tl_on:
        logger.info("🔀 Translation layer %s→%s gender=%s", tl_from, tl_to, tl_gen)
    ...
    async def process_turn(pcm: np.ndarray) -> None:
        ...
        async with lock:
            ...
            llm_text = await tl_translate_user(user_text, tl_from, tl_to, loop) if tl_on else user_text
            history.append({"role": "user", "text": llm_text})
            ...
            eff_lang = tl_to if tl_on else lang
            llm_fut = loop.run_in_executor(None, lambda: llm_fn(
                hist_snap, eff_lang, voice_stem, rag_context, customer_ctx, custom_prompt_text
            ))
            ...
            out_text  = await tl_translate_agent(ai_text, tl_to, tl_from, loop) if tl_on else ai_text
            out_voice = select_voice_by_gender(registry, tl_from, tl_gen) if tl_on else voice_stem
            out_lang  = tl_from if tl_on else lang
            tts_text = _humanize_text(out_text, out_lang)
            try:
                wav = await tts(tts_text, out_lang, out_voice or voice_stem)
                ...
                a64 = ""
            try:
                await ws.send_json({"type": "response", "text": out_text, "audio": a64})
