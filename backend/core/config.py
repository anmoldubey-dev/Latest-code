...
# ── Load services.config before computing any constant ───────────
# This runs when config.py is first imported — guaranteed to be
# before any os.environ.get() call in this file.
_svc = PROJECT_ROOT / "services.config"
if _svc.exists():
    for _ln in _svc.read_text().splitlines():
        _ln = _ln.strip()
