...
async def lifespan(app):
    ...
    # [Sentiment] Kick off background ML model load (VADER ready instantly, ML model ~5-15s)
    try:
        from backend.api.sentiment_engine import SentimentEngine
        SentimentEngine.get()
        logger.info("[Sentiment] Engine warm-up initiated — VADER ready, ML model loading in background.")
    except Exception as exc:
        logger.warning("[Sentiment] Engine warm-up failed: %s", exc)
