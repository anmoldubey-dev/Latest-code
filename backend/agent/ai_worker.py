    ...
    async for event in stream:
        ...
        if session.buf.ready() and not session.lock.locked():
            ...
            if pcm_utt is not None:
                ...
                # Only clear TTS / barge-in if speech energy is real (not ambient noise).
                # The STT floor is 0.040; use 0.035 here so we don't cut Emma for room noise.
                rms = float(np.sqrt(np.mean(pcm_utt.astype(np.float32) ** 2)))
                if rms > 0.035:
...
async def _post_call_cleanup(session: LiveKitSession) -> None:
    ...
    try:
        ...
        ltm = _m.get("long_term_memory")
        _phones = session.session_mem.get_entities_aggregate().get("phones", []) if session.session_mem else []
        phone = _phones[0] if _phones else ""
        if ltm and phone:
            await loop.run_in_executor(
        ...
        # Save to pgvector (Neon) so admin console can fetch this call
        if _m.get("pg_memory") and session.history:
            import time as _time
            duration_secs = round(_time.time() - (session.session_mem.start_time if session.session_mem else _time.time()), 1)
            session_turns = [{"role": t["role"], "text": t["text"]} for t in session.history]
            pgm = _m["pg_memory"]
            try:
                await loop.run_in_executor(
                    None,
                    pgm.save_call_record,
                    session.session_id, phone, session.lang,
                    session_turns, [],
                    summary.get("sentiment", "neutral"), summary.get("summary", ""),
                    duration_secs, getattr(session, "voice_name", ""), getattr(session, "llm_key", ""),
                )
                logger.info("[Cleanup] pgvector saved  session=%s", session.session_id[:8])
            except Exception as exc:
                logger.warning("[Cleanup] pgvector save failed (session=%s): %s", session.session_id[:8], exc)
            # Save individual turns to conversation_turns (feeds Memory Explorer)
            for t in session.history:
                try:
                    await loop.run_in_executor(
                        None, pgm.save_turn,
                        session.session_id, t["role"], t["text"], session.lang,
                    )
                except Exception:
                    pass
