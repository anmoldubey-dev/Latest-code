...
async def _dynamic_wait_sec(_department: str, position: int) -> int:
    """Returns estimated wait in SECONDS = position × base_seconds_per_slot."""
    try:
...
async def dequeue_caller(session_id: str,
    ...
    # Remove immediately from in-memory queue (don't wait for Kafka consumer)
    ...
    if reason == "abandoned":
        ...
        # Call is being picked up by an agent — only record wait time, keep status active
        await db.update_call_log_status(
            session_id, "connected", wait_seconds=wait_sec
        )
    else:
        await db.update_call_log_status(
...
async def _synthesize_wav(text: str) -> Optional[bytes]:
    """Piper TTS → WAV bytes; returns None on failure."""
    try:
        import tempfile as _tf, os as _os
        _piper = "/home/swayam/miniconda3/envs/serviceA/bin/piper"
        _model = "/home/swayam/Documents/VoiceAicore/UI_01/backend/piper/models/en_US-ryan-high.onnx"
        with _tf.NamedTemporaryFile(suffix=".wav", delete=False) as _f:
            _out = _f.name
        proc = await asyncio.create_subprocess_exec(
            _piper, "--model", _model, "--output_file", _out,
...
async def _deliver_tts_to_room(room_name: str, text: str, retries: int = 5, retry_delay: float = 3.0):
    """
    Synthesise text with Piper → WAV bytes pushed via LiveKit SendData.
    Retries up to `retries` times (room may not exist yet when caller first joins).
    Falls back to tts_queue_text (browser speechSynthesis) if Piper fails.
    """
    from livekit.api import LiveKitAPI
    from livekit.protocol.room import SendDataRequest

    wav_bytes = await _synthesize_wav(text)
        ...
        try:
            ...
            if "not_found" in msg or "does not exist" in msg:
                if attempt < retries - 1:
                    logger.debug("[TTS] room %s not ready, retry %d/%d in %.0fs", room_name, attempt + 1, retries, retry_delay)
                    await asyncio.sleep(retry_delay)
                else:
                    logger.warning("[TTS] room %s never appeared after %d retries", room_name, retries)
            else:
                logger.warning("[TTS] send_data failed for room %s: %s", room_name, exc)
                return
        finally:
            await lk.aclose()
...
async def _countdown_loop(session_id: str, room_name: str, department: str):
    ...
    """
    await asyncio.sleep(5)  # give the caller time to fully connect to LiveKit

    # If human agent already accepted during the 5s window, don't interrupt with AI IVR
    if not find_caller(session_id):
        return

    # AI routing prompt — sent once at the start so caller can speak their language
    ai_cfg = await db.get_config("ai_agents_enabled") or ""
    if ai_cfg.lower() in ("1", "true", "yes"):
        await _deliver_tts_to_room(
            room_name,
            "Please speak your preferred language and we will connect you to the right assistant.",
        )
        await asyncio.sleep(4)  # let the prompt finish before queue position
    ...
    while find_caller(session_id):
        ...
        wait_secs = await _dynamic_wait_sec(dept, pos) if dept else pos * _WAIT_PER_CALLER_SEC
