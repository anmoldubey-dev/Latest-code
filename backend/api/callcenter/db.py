...
async def init_db():
    ...
    CREATE TABLE IF NOT EXISTS transcripts (
        id         SERIAL PRIMARY KEY,
        call_id    INTEGER REFERENCES cc_sessions(id) ON DELETE SET NULL,
    ...
    async with pool.acquire() as conn:
        ...
        # Ensure unique constraint on cc_callers.email (missing on pre-existing tables)
        await conn.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint
                    WHERE conrelid = 'cc_callers'::regclass AND contype = 'u'
                    AND conname = 'cc_callers_email_key'
                ) THEN
                    ALTER TABLE cc_callers ADD CONSTRAINT cc_callers_email_key UNIQUE (email);
                END IF;
            END $$;
        """)
        # [Recording] Add recording columns to cc_sessions if not already present (safe on existing tables)
        await conn.execute("""
            DO $$ BEGIN
            ...
            END $$;
        """)
        # Ensure unique constraint on agent_states.agent_identity (missing on pre-existing tables)
        await conn.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint
                    WHERE conrelid = 'agent_states'::regclass AND contype = 'u'
                    AND conname = 'agent_states_agent_identity_key'
                ) THEN
                    ALTER TABLE agent_states ADD CONSTRAINT agent_states_agent_identity_key UNIQUE (agent_identity);
                END IF;
            END $$;
        """)
        # Sync sequences in case table was pre-populated (prevents duplicate key errors)
        await conn.execute("""
            SELECT setval('cc_callers_id_seq', GREATEST((SELECT COALESCE(MAX(id),0) FROM cc_callers), 1));
        """)
        await conn.execute("""
            SELECT setval('cc_sessions_id_seq', GREATEST((SELECT COALESCE(MAX(id),0) FROM cc_sessions), 1));
        """)
        await conn.execute("""
            SELECT setval('outbound_queue_id_seq', GREATEST((SELECT COALESCE(MAX(id),0) FROM outbound_queue), 1));
        """)
        ...
        await conn.execute("""
            INSERT INTO admin_config (key, value) VALUES
                ...
                ('zoho_client_id',       '1000.KEGK4NPNXAK7HI9937UQTG8O29LQRT'),
                ('zoho_client_secret',   '62ce2efc32ac6f093f82bde9044ba9fee327300fd2'),
                ('zoho_access_token',    '1000.635bf2ef4a141934b25dfa59b03e6061.dbceee4b167a2fdf2ed0a67a8e67c357'),
                ('zoho_refresh_token',   '1000.fe248f6dd2e57d5925f0236c69fcb9b9.8f0a8c23c2918da4d717869f49cdab00'),
                ('zoho_api_domain',      'zohoapis.in'),
                ('zoho_accounts_domain', 'accounts.zoho.in'),
                ('zoho_desk_org_id',     '60068409046'),
                ('zoho_desk_dept_id',    '255404000000010772'),
                ('zoho_mail_from',       'srcom_soft@zohomail.in'),
                ('zoho_mail_account_id', '1819144000000002002'),
                ('zoho_webhook_secret',  'srcomsoft_webhook_2024'),
...
async def update_call_log_status(session_id: str, status: str, **kwargs):
    ...
    if "ended_at" in kwargs:
        ...
        idx += 1
    elif status in ("completed", "abandoned", "ended", "cancelled"):
        sets.append(f"ended_at = ${idx}")
        params.append(datetime.now(timezone.utc))
        idx += 1
    elif status in ("connected", "on_hold", "conference"):
        # Clear ended_at so the call stays visible in the active-calls view
        sets.append(f"ended_at = NULL")

    query = f"UPDATE cc_sessions SET {', '.join(sets)} WHERE session_id = $1 OR room_id = $1"
    async with pool.acquire() as conn:
