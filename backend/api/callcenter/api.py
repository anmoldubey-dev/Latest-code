...
from .event_hub import event_hub, is_user_online
from .token_service import generate_token, LIVEKIT_URL

# [Direct Call] HTTP-based user presence: email → last heartbeat timestamp (epoch seconds).
# This is the primary presence signal — more reliable than WS-only tracking because it
# works even when the WS URL or connection is misconfigured.
_user_http_heartbeats: dict[str, float] = {}
_USER_ONLINE_TTL = 90  # seconds — user is considered online if heartbeat within this window


def _is_user_online_combined(email: str) -> bool:
    """[Direct Call] User is online if WS-registered OR HTTP heartbeat within 90s."""
    e = email.lower().strip()
    http_ok = (time.time() - _user_http_heartbeats.get(e, 0)) < _USER_ONLINE_TTL
    return http_ok or is_user_online(e)
...
# [Direct Call] Request model for agent-initiated direct peer-to-peer call
...
# ─────────────────────────────────────────────────────────────────────────────
# [Direct Call] POST /cc/outbound/direct
# Agent enters a caller's email in their browser call panel. This endpoint:
#   1. Checks if the caller is currently online (has an open WS dashboard session)
#   2. Creates a LiveKit room and generates tokens for both parties
#   3. Sends an incoming_direct_call WS event to the caller's dashboard
#   4. Returns the agent's token so they can join the room immediately
# ─────────────────────────────────────────────────────────────────────────────

@cc_router.post("/outbound/direct")
async def initiate_direct_call(req: DirectCallRequest):
    caller_email = req.caller_email.strip().lower()

    # [Direct Call] Check presence via combined HTTP heartbeat + WS registration
    if not _is_user_online_combined(caller_email):
        return {"online": False, "message": f"{caller_email} is not currently online."}

    # [Direct Call] Create a unique room for this call
    room_id = f"direct-{uuid.uuid4().hex[:10]}"

    # [Direct Call] Generate LiveKit token for the agent
    ...
    # [Direct Call] Generate LiveKit token for the caller (sent via WS event)
    caller_identity = f"caller-{caller_email.split('@')[0]}-{uuid.uuid4().hex[:4]}"
    caller_token = generate_token(
        room_name=room_id,
        identity=caller_identity,
        name=caller_email,
    )

    # [Direct Call] Persist a cc_sessions row so the call appears in history
    try:
        pool = await db.get_pool()
        async with pool.acquire() as conn:
            # Upsert caller into cc_callers
            caller_row = await conn.fetchrow(
                """INSERT INTO cc_callers (email, display_name, last_seen)
                   ...
                   RETURNING id""",
                caller_email, caller_email.split('@')[0],
            )
            caller_id = caller_row["id"] if caller_row else None
            await conn.execute(
                """INSERT INTO cc_sessions
                       (session_id, room_id, caller_id, department, status, agent_id, created_at)
                   VALUES ($1, $2, $3, $4, 'connected', $5, NOW())""",
                f"direct-{uuid.uuid4().hex[:8]}", room_id, caller_id,
                req.department or "General", req.agent_identity,
            )
    except Exception as _e:
        logger.warning("[Direct Call] Failed to persist cc_sessions: %s", _e)

    # [Direct Call] Publish incoming call event — target_email ensures delivery
    # only to the specific user's WS connection (filtered in ws_router._sender)
    await event_hub.publish({
    ...
    logger.info("[Direct Call] Agent %s calling %s in room %s", req.agent_identity, caller_email, room_id)
...
# ─────────────────────────────────────────────────────────────────────────────
# [Direct Call] POST /cc/outbound/direct-decline
# Called when the target user declines the incoming direct call popup.
# Notifies the agent so they know the call was rejected.
# ─────────────────────────────────────────────────────────────────────────────

@cc_router.post("/outbound/direct-decline")
async def decline_direct_call(payload: dict):
    # [Direct Call] Broadcast decline event so agent UI updates accordingly
    ...
    logger.info("[Direct Call] %s declined call from agent %s", caller_email, agent_identity)
...
async def agent_heartbeat(req: HeartbeatRequest):
    ...
    Returns 200 even when the DB is unavailable so the browser console stays clean.
    """
    try:
        pool = await db.get_pool()
        async with pool.acquire() as conn:
        ...
        # DB unavailable — return OK so the client doesn't log console errors
        return {"status": "db_unavailable", "agent_identity": req.agent_identity}


# ─────────────────────────────────────────────────────────────────────────────
# [Direct Call] POST /cc/user/heartbeat
# User dashboard calls this every 30s to register itself as "online".
# This is the primary presence mechanism — more reliable than WS-based tracking
# because it works regardless of WebSocket connection state.
# ─────────────────────────────────────────────────────────────────────────────
@cc_router.post("/user/heartbeat")
async def user_heartbeat(payload: dict):
    email = (payload.get("email") or "").strip().lower()
    if email:
        _user_http_heartbeats[email] = time.time()
    return {"status": "ok"}
